## Объединенное описание API "Поисковая платформа" РОСПАТЕНТ

Этот документ объединяет описание API "Поисковая платформа" РОСПАТЕНТ и подробное описание запроса для поиска релевантных патентов по тексту, **включая важное уточнение о минимальной длине текста**.
...
*   **Поиск похожих патентных документов (POST /similar_search):**
    ...
    *   **Структура ответа (JSON):**
        *   `total`: Общее количество найденных документов.
        *   `available`: Количество доступных документов.
        *   `hits`: Массив документов, где каждый документ содержит общую информацию, библиографические данные, метаданные, а также фрагменты текста и информацию о релевантности.
...
**5. Поиск релевантных патентов по текстовому описанию**
