...
*   **Патентный поиск (POST /search):**
    *   Позволяет осуществлять поиск патентных документов с использованием различных параметров.
...
*   **Предоставление данных патентного документа (GET /docs/<идентификатор документа>):**
    ...
    *   Структура ответа (JSON): полные данные патентного документа (см. п. 5.2).
*   **Поиск похожих патентных документов (POST /similar_search):**
    *   Позволяет искать похожие патенты, используя либо идентификатор известного патента, либо фрагмент текста.
    *   **Параметры запроса (в формате JSON):**
    ...
    *   **Структура ответа (JSON):**
        *   `total`: Общее количество найденных документов.
         *   `pages`: Общее количество страниц результатов.
        *   `next_page`: URL следующей страницы (если есть).
        *   `prev_page`: URL предыдущей страницы (если есть).
        *   `data`: Массив документов, где каждый документ содержит общую информацию, библиографические данные, метаданные, а также фрагменты текста и информацию о релевантности.
...
1.  Пользователь API выполняет поиск (например, `POST /similar_search` с `type_search="text_search"` или `POST /search`).
2.  Результаты поиска всегда содержат `id` найденных документов (поле `id` в объекте `hits` или `data`).
...
4.  Далее полученные `id` могут использоваться для получения полных данных патента (см. п. 5.2) или для поиска похожих документов (используя `POST /similar_search` с `type_search="id_search"`).

**5. Поиск релевантных патентов по текстовому описанию и получение полных данных**

**5.1. Поиск по текстовому описанию:**
...
**5.2. Получение полных данных патента:**

После получения списка результатов поиска, для каждого патента вам нужно выполнить отдельный GET запрос к эндпоинту `/docs/{id}`.

**Пример запроса (cURL):**

```bash
curl --location --request GET 'https://searchplatform.rospatent.gov.ru/patsearch/v0.2/docs/RU2359130C1_20090620' \
--header 'Authorization: Bearer YOUR_API_KEY' \
--header 'Content-Type: application/json'
```

*   Замените `RU2359130C1_20090620` на `id`  интересующего вас патента.
...
{
  "common": {
    // ... (общая информация о документе, включая дату публикации в "publication_date")
  },
  "meta": {
    // ... (метаданные)
  },
...
**5.3. Извлечение даты и текста:**

После получения JSON ответа для каждого патента:

*   Дата публикации:  Используйте поле `common.publication_date`.
*   Полный текст описания изобретения:  Используйте поле `description.{lang}` (например, `description.ru` для русского языка). **Поле может содержать XML или HTML теги.**.
*   Формула изобретения:  Используйте поле `claims.{lang}` (например, `claims.ru` для русского языка). **Поле может содержать XML или HTML теги.**
*   Реферат изобретения:  Используйте поле `abstract.{lang}` (например, `abstract.ru` для русского языка). **Поле может содержать XML или HTML теги.**

**6. Приложение 1. Операторы и синтаксис языка запросов**
...
**7. Приложение 2. Формат данных патентного документа**

Данные патентного документа возвращаются в формате JSON и включают поля: `common`, `meta`, `biblio.{lang}`, `abstract.{lang}`, `claims.{lang}`, `description.{lang}`, `drawings`. **Обратите внимание, что поля `abstract`, `claims` и `description` могут содержать XML и HTML теги, которые необходимо будет обрабатывать и удалять при необходимости на стороне вашего приложения.**
